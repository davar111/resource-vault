const dict = {
  ru: {
    brand: "Vault",
    nav: "Навигация",
    navAll: "Все ссылки",
    navFav: "Избранное",
    navHidden: "Скрытые",
    navRecent: "Недавние",
    navSpace: "Space",
    navLiquid: "Liquid Lab",
    collections: "Коллекции",
    savedFilters: "Сохраненные фильтры",
    collectionSingle: "Коллекция",

    addLink: "+ Добавить ссылку",
    newCollection: "Создать коллекцию",
    saveFilter: "Сохранить фильтр",
    saveFilterShort: "Сохранить",
    exportJson: "Экспорт JSON",
    importJson: "Импорт JSON",
    settings: "Настройки",
    localHint: "Данные ссылок хранятся в Supabase.",
    signInGoogle: "Войти через Google",
    signOut: "Выйти",
    authSignedOut: "Вход не выполнен",
    authSignedInAs: "Вошли как",
    authGateShowcaseTitle: "Храните ссылки и коллекции в одном месте",
    authGateShowcaseText: "Доступно после регистрации",
    authGateTitle: "Вход",
    authGateText: "Чтобы управлять ссылками и коллекциями, войдите через Google.",
    authGateBtn: "Войти через Google",
    authGateGuestBtn: "Продолжить как гость",
    guestModeStatus: "Гостевой режим",
    guestModeHint: "Гостевой режим: доступен просмотр, создание и изменение отключены.",

    all: "Все ссылки",
    favorites: "Избранное",
    hidden: "Скрытые",
    recent: "Недавние",
    spaceTitle: "Space",
    spaceOpen: "Открыть",
    spaceArchive: "Архивировать",
    spaceBackToLibrary: "В библиотеку",
    spaceDoneTitle: "На сегодня разобрано",
    spaceDoneText: "Добавляй новые ссылки или возвращайся завтра.",
    spaceSwipeHint: "Свайп влево — архив, вправо — открыть",
    spaceNoNote: "Заметки нет",
    spaceStreakLabel: "Серия",
    liquidTitle: "Liquid Lab",
    liquidMeta: "Наведи курсор на текст, чтобы увидеть эффект текучей краски",
    liquidHeroText: "MONOLIT",
    liquidFallbackTitle: "WebGL недоступен",
    liquidFallbackText: "Для этой вкладки нужна поддержка WebGL в браузере.",
    items: "элементов",
    contextManual: "Ручная коллекция",

    searchPlaceholder: "Поиск по всему — URL, названию, тегам, заметкам...",
    filters: "Фильтры",
    themeLight: "Светлая",
    themeDark: "Темная",
    switchThemeToLight: "Включить светлую тему",
    switchThemeToDark: "Включить темную тему",

    sortNewest: "Сначала новые",
    sortOldest: "Сначала старые",
    sortTitleAsc: "Название А-Я",
    sortTitleDesc: "Название Я-А",
    sortSource: "По источнику",

    filterTypes: "Типы",
    filterSources: "Источники",
    filterTag: "Тег содержит",
    filterFavoriteOnly: "Только избранное",
    clearFilters: "Сбросить всё",
    chipSearch: "Поиск",

    modalAddTitle: "Добавить ссылку",
    modalEditTitle: "Редактировать ссылку",
    modalUrl: "URL",
    modalTitle: "Название",
    modalTitleHint: "Необязательно",
    modalTags: "Теги (через запятую)",
    modalType: "Тип",
    modalSource: "Источник",
    modalNote: "Заметка",
    noteEmpty: "Без описания",
    addTo: "Добавить в коллекции",
    addToInbox: "Без коллекции",
    markFavorite: "Избранное",
    cancel: "Отмена",
    save: "Сохранить",
    saveChanges: "Сохранить изменения",

    modalCollectionTitle: "Создать коллекцию",
    name: "Название",
    description: "Описание",
    sharedCollection: "Совместная коллекция",
    sharedBadge: "Совместная",
    sharedBadgeCompact: "совм.",
    inviteToCollection: "Пригласить",
    invitePrompt: "Email участника",
    inviteInvalidEmail: "Введите корректный email.",
    inviteSelfError: "Нельзя пригласить свою же почту.",
    inviteSent: "Приглашение отправлено.",
    create: "Создать",

    saveFilterTitle: "Сохранить фильтр",
    saveFilterPrompt: "Название фильтра",

    open: "Открыть",
    edit: "Редактировать",
    del: "Удалить",
    deleteLinkTitle: "Удалить ссылку?",
    deleteLinkText: "Это действие нельзя отменить.",
    deleteSavedFilterTitle: "Удалить фильтр?",
    deleteSavedFilterText: "Сохраненный фильтр будет удален.",
    deleteCollectionTitle: "Удалить коллекцию?",
    deleteCollectionText: "Коллекция будет удалена. Ссылки останутся в Vault.",
    deleteCollection: "Удалить коллекцию",
    leaveCollection: "Выйти из коллекции",
    leaveCollectionTitle: "Выйти из коллекции?",
    leaveCollectionText: "Вы потеряете доступ к этой коллекции. У владельца она останется.",
    leaveCollectionSuccess: "Вы вышли из коллекции.",
    renameCollection: "Переименовать",
    pinCollection: "Закрепить коллекцию",
    unpinCollection: "Открепить коллекцию",
    renameCollectionPrompt: "Новое название коллекции",
    deleteCollectionConfirm: "Удалить коллекцию без возможности восстановления?",

    emptyAllTitle: "Пока нет ссылок",
    emptyAllText: "Добавьте первую ссылку, чтобы начать.",
    emptyCollectionTitle: "В коллекции пусто",
    emptyCollectionText: "Добавьте ссылки и назначьте эту коллекцию вручную.",
    emptyFavTitle: "Нет избранного",
    emptyFavText: "Отмечайте ссылки сердцем, чтобы видеть их здесь.",
    emptyHiddenTitle: "Скрытый раздел пуст",
    emptyHiddenText: "Добавляйте сюда личные ссылки, они не видны в остальных разделах.",
    emptyRecentTitle: "История пуста",
    emptyRecentText: "Открывайте ссылки, и они появятся здесь.",
    hiddenSetPasswordPrompt: "Задайте пароль для скрытого раздела (минимум 4 символа)",
    hiddenConfirmPasswordPrompt: "Повторите пароль",
    hiddenEnterPasswordPrompt: "Введите пароль скрытого раздела",
    hiddenPasswordMismatch: "Пароли не совпадают.",
    hiddenPasswordTooShort: "Пароль должен быть не короче 4 символов.",
    hiddenWrongPassword: "Неверный пароль.",
    hiddenAuthCreateTitle: "Пароль для скрытого",
    hiddenAuthCreateText: "Создайте пароль для доступа к разделу Скрытые.",
    hiddenAuthUnlockTitle: "Доступ к скрытому",
    hiddenAuthUnlockText: "Введите пароль, чтобы открыть раздел Скрытые.",
    hiddenAuthPasswordLabel: "Пароль",
    hiddenAuthConfirmLabel: "Повтор пароля",
    demoHintText: "✦ Это примеры ссылок. Добавь свою первую, чтобы начать коллекцию.",
    demoHintAction: "Добавить ссылку",
    demoBadge: "пример",

    quickAdd: "Добавить ссылку",
    quickAddHint: "Быстрое добавление",
    expandNote: "Показать",

    chipType: "Тип",
    chipSource: "Источник",
    chipTag: "Тег",
    chipFav: "Избранное",
    anyOption: "Любой",

    type_case: "Кейс",
    type_inspiration: "Референс",
    type_article: "Статья",
    type_tool: "Инструмент",
    type_asset: "Ассет",

    source_site: "Сайт",
    source_behance: "Behance",
    source_awwwards: "Awwwards",
    source_pinterest: "Pinterest",
    source_dribbble: "Dribbble",
    source_other: "Другое"
    ,
    authRequired: "Сначала войдите через Google."
    ,
    authRequiredGuest: "В гостевом режиме создание и изменение недоступны. Войдите через Google.",
    sessionExpired: "Сессия истекла, войдите снова."
  },

  en: {
    brand: "Vault",
    nav: "Navigation",
    navAll: "All links",
    navFav: "Favorites",
    navHidden: "Hidden",
    navRecent: "Recent",
    navSpace: "Space",
    navLiquid: "Liquid Lab",
    collections: "Collections",
    savedFilters: "Saved Filters",
    collectionSingle: "Collection",

    addLink: "+ Add link",
    newCollection: "Create collection",
    saveFilter: "Save filter",
    saveFilterShort: "Save",
    exportJson: "Export JSON",
    importJson: "Import JSON",
    settings: "Settings",
    localHint: "Link data is stored in Supabase.",
    signInGoogle: "Sign in with Google",
    signOut: "Sign out",
    authSignedOut: "Not signed in",
    authSignedInAs: "Signed in as",
    authGateShowcaseTitle: "Keep links and collections in one place",
    authGateShowcaseText: "Available after sign up",
    authGateTitle: "Sign in",
    authGateText: "Sign in with Google to manage links and collections.",
    authGateBtn: "Sign in with Google",
    authGateGuestBtn: "Continue as guest",
    guestModeStatus: "Guest mode",
    guestModeHint: "Guest mode: browsing is available, creating and editing are disabled.",

    all: "All links",
    favorites: "Favorites",
    hidden: "Hidden",
    recent: "Recent",
    spaceTitle: "Space",
    spaceOpen: "Open",
    spaceArchive: "Archive",
    spaceBackToLibrary: "Back to library",
    spaceDoneTitle: "Done for today",
    spaceDoneText: "Add more links or come back tomorrow.",
    spaceSwipeHint: "Swipe left to archive, right to open",
    spaceNoNote: "No note",
    spaceStreakLabel: "Streak",
    liquidTitle: "Liquid Lab",
    liquidMeta: "Move the cursor over the text to reveal fluid ink blur",
    liquidHeroText: "MONOLIT",
    liquidFallbackTitle: "WebGL is unavailable",
    liquidFallbackText: "This tab requires WebGL support in your browser.",
    items: "items",
    contextManual: "Manual collection",

    searchPlaceholder: "Search across URL, title, tags and notes...",
    filters: "Filters",
    themeLight: "Light",
    themeDark: "Dark",
    switchThemeToLight: "Switch to light theme",
    switchThemeToDark: "Switch to dark theme",

    sortNewest: "Newest first",
    sortOldest: "Oldest first",
    sortTitleAsc: "Title A-Z",
    sortTitleDesc: "Title Z-A",
    sortSource: "By source",

    filterTypes: "Types",
    filterSources: "Sources",
    filterTag: "Tag contains",
    filterFavoriteOnly: "Only favorites",
    clearFilters: "Clear all",
    chipSearch: "Search",

    modalAddTitle: "Add link",
    modalEditTitle: "Edit link",
    modalUrl: "URL",
    modalTitle: "Title",
    modalTitleHint: "Optional",
    modalTags: "Tags (comma)",
    modalType: "Type",
    modalSource: "Source",
    modalNote: "Note",
    noteEmpty: "No description",
    addTo: "Add to collections",
    addToInbox: "Unsorted",
    markFavorite: "Favorite",
    cancel: "Cancel",
    save: "Save",
    saveChanges: "Save changes",

    modalCollectionTitle: "Create collection",
    name: "Name",
    description: "Description",
    sharedCollection: "Shared collection",
    sharedBadge: "Shared",
    sharedBadgeCompact: "shared",
    inviteToCollection: "Invite",
    invitePrompt: "Teammate email",
    inviteInvalidEmail: "Enter a valid email.",
    inviteSelfError: "You cannot invite your own email.",
    inviteSent: "Invitation sent.",
    create: "Create",

    saveFilterTitle: "Save filter",
    saveFilterPrompt: "Filter name",

    open: "Open",
    edit: "Edit",
    del: "Delete",
    deleteLinkTitle: "Delete link?",
    deleteLinkText: "This action cannot be undone.",
    deleteSavedFilterTitle: "Delete filter?",
    deleteSavedFilterText: "Saved filter will be removed.",
    deleteCollectionTitle: "Delete collection?",
    deleteCollectionText: "Collection will be removed. Links will stay in Vault.",
    deleteCollection: "Delete collection",
    leaveCollection: "Leave collection",
    leaveCollectionTitle: "Leave collection?",
    leaveCollectionText: "You will lose access to this collection. It will stay for the owner.",
    leaveCollectionSuccess: "You left the collection.",
    renameCollection: "Rename",
    pinCollection: "Pin collection",
    unpinCollection: "Unpin collection",
    renameCollectionPrompt: "New collection name",
    deleteCollectionConfirm: "Delete this collection permanently?",

    emptyAllTitle: "No links yet",
    emptyAllText: "Add your first link to start.",
    emptyCollectionTitle: "Collection is empty",
    emptyCollectionText: "Add links and assign this collection manually.",
    emptyFavTitle: "No favorites",
    emptyFavText: "Mark links with heart to see them here.",
    emptyHiddenTitle: "Hidden is empty",
    emptyHiddenText: "Add private links here. They stay out of other sections.",
    emptyRecentTitle: "History is empty",
    emptyRecentText: "Open links and they will appear here.",
    hiddenSetPasswordPrompt: "Set a password for Hidden (min 4 chars)",
    hiddenConfirmPasswordPrompt: "Confirm password",
    hiddenEnterPasswordPrompt: "Enter Hidden password",
    hiddenPasswordMismatch: "Passwords do not match.",
    hiddenPasswordTooShort: "Password must be at least 4 characters.",
    hiddenWrongPassword: "Wrong password.",
    hiddenAuthCreateTitle: "Hidden password",
    hiddenAuthCreateText: "Create a password to access the Hidden section.",
    hiddenAuthUnlockTitle: "Hidden access",
    hiddenAuthUnlockText: "Enter password to open Hidden section.",
    hiddenAuthPasswordLabel: "Password",
    hiddenAuthConfirmLabel: "Confirm password",
    demoHintText: "✦ These are sample links. Add your first one to start your own vault.",
    demoHintAction: "Add link",
    demoBadge: "demo",

    quickAdd: "Add link",
    quickAddHint: "Quick add",
    expandNote: "Show",

    chipType: "Type",
    chipSource: "Source",
    chipTag: "Tag",
    chipFav: "Favorite",
    anyOption: "Any",

    type_case: "Case",
    type_inspiration: "Inspiration",
    type_article: "Article",
    type_tool: "Tool",
    type_asset: "Asset",

    source_site: "Site",
    source_behance: "Behance",
    source_awwwards: "Awwwards",
    source_pinterest: "Pinterest",
    source_dribbble: "Dribbble",
    source_other: "Other",
    authRequired: "Sign in with Google first.",
    authRequiredGuest: "Guest mode is read-only. Sign in with Google to create or edit.",
    sessionExpired: "Session expired. Please sign in again."
  }
};

export function t(lang, key) {
  return dict?.[lang]?.[key] ?? dict.en[key] ?? key;
}
